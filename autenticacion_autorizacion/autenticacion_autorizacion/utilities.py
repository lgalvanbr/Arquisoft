...
class DetectorAnomalias:
    ...
    def registrar_intento_fallido(username, direccion_ip, user_agent=None, motivo='Contraseña incorrecta'):
        ...
        # Verificar si hay bloqueo activo (< 2 segundos)
        ...
        # Si hay más de X intentos fallidos, registrar anomalía
    ...
    def registrar_intento_exitoso(usuario, direccion_ip, user_agent=None):
        ...
        # Desbloquear si existe bloqueo previo
    ...
    def _esta_bloqueado(username, direccion_ip):
        ...
        Implementación optimizada para < 2 segundos
        """
        # Buscar primero por IP (más común en ataques de fuerza bruta)
        ...
        # Si no hay bloqueo por IP, buscar por username
        bloqueo = IntentoBloqueado.objects.filter(
            username=username,
            activo=True,
            fecha_bloqueo_hasta__gt=timezone.now()
